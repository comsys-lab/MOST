...
int ssd2gpu_BWgiveIndx_OverSub(long tensor_size, int start_index, int needed_index){
    long rest_size = tensor_size;
    int curr_index = needed_index - 1;
    int prefetch_scheduled_index = -1;
    ...
    if (curr_index == 0 || curr_index < start_index)
    {
        curr_index = needed_index - 1;
    }
    return curr_index;
}
...
int ssd2gpu_BWgiveIndx_OverSub_v2(long tensor_size, int start_index, int final_index, double safe_time, double ssd_movement_time){
    //long rest_size = tensor_size;
    //implementation 3
    //start_index : eviction finished index
    //final_index : kernel index, which requires the tensor
    //safe_index : safe kernel index to hide prefetching latency
    //as increaseing kernel index from start_index to final_index, calculate overhead and delay
    int curr_index = final_index - 1;
    double prefetch_delay = 0;
    bool is_over = false;
    int finish_index = -1;
    long oversubscription_size = 0;
    double oversubscription_delay = 0;
    long GPU_total_cap = (long)(GPU_memory_size_GB * 1024 * 1024 * 1024);
    int prefetched_index = -1;
    double lowest_delay = 0;
    double delay_diff = 0;
    int prefetch_scheduled_index = -1;
    ...
    {   
        ...
        // 1. calculate prefetching finish index
        std::vector<long> temp_memory_estimation(GPU_resident_memory_estimation);
        double finish_time = kernel_time_table[j] + ssd_movement_time;
        //for (int k = j; k < kernel_time_table_extended.size(); k++)
        for (int k = j; k < kernel_time_table.size(); k++)
        {
            if (kernel_time_table[k] <= finish_time && kernel_time_table[k+1] > finish_time)
            {
                finish_index = k;
                break;
            }
        ...
        // 2. calculate oversubscription overhead after prefetching
        // if final < finish => finish
        // if finish < final => final
        if (final_index < finish_index){
            prefetched_index = finish_index;
        }
        else{
            prefetched_index = final_index;
        }
        for (int m = j; m < prefetched_index; m++)
        {   
            temp_memory_estimation[m] += tensor_size;
        }
        
        for (int n = j; n < kernel_time_table.size(); n++)
        {
            if (temp_memory_estimation[n] - GPU_total_cap > 0)
            {
                oversubscription_size += temp_memory_estimation[n] - GPU_total_cap;
            }
        }
        oversubscription_delay = oversubscription_size / SSD_PCIe_bandwidth_GBps;
        
        // 3. calculate the delay due to late prefetching
        if (finish_time < safe_time){
            prefetch_delay = 0;
        }
        else{
            prefetch_delay = finish_time - safe_time;
        }

        // 4. calculate the difference between the prefetch delay and oversubscription delay

        //print_GPU_mem_estimation_temp(temp_memory_estimation);

        //4. select the index shows the lowest delay
        
        if (lowest_delay > prefetch_delay)
        {   
            lowest_delay = prefetch_delay;
            prefetch_scheduled_index = j;
        }

        if(prefetch_scheduled_index < 0)
        {
            std::cout<<"----------------------------------------------------"<<std::endl;
            std::cout<<"Error occurs when scheduling starting index:"<<start_index<<std::endl;
            std::cout<<"----------------------------------------------------"<<std::endl;
        }

    }
    
    return prefetch_scheduled_index;

    // delay, oversubscription overhead
    // need to calculate when is the optimal index for lowest delay
...
void scheduling_prefetch(){
    ...
    for (int i = 0; i < tot_iter_num; i++)
    {
        ...
        // implementation 3, sorting tensors only with the size and the inactive time
        //Sort the intervals
        std::sort(interval_list.begin(), interval_list.end(), [](Hidding_Interval* a, Hidding_Interval* b){
        ...
        {   
            ...
            {   
                ...
                double eviction_finish_time = kernel_time_table[curr_interval->kernelLevel_interval[0]] + ssd_movement_estimated_time + delayed_time;
                ...
                // implementation 3
                // sum up all oversubscription overhead 
                ...
                {   
                    ...
                    //get prefetch_safe_index and prefetch_finish_index
                    ...
                    if(pcie_prefetch_index > pcie_eviction_clear_index){
                        ...
                        //implementation 2: ssd time
                        // if (curr_interval->time_estimated < ssd_safe_time)
                        // {
                        //     delayed_time += ssd_safe_time - curr_interval->time_estimated;
                        // }
...
void print_GPU_mem_estimation_temp(std::vector<long> temp_mem){
    std::cout<<"This is temporal estimiation"<<std::endl;
    for (int i = 0; i < kernel_list.size(); i++)
    {
        std::cout<<"Kernel "<<i<<": "<<temp_mem[i]<<std::endl;
    }
