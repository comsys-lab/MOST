...
models = {# "Inceptionv3"   : ("Inceptionv3",       "Inceptionv3",      "Inceptionv3",    [512, 768, 1024, 1152, 1280, 1408, 1536, 1664, 1792]), 
          ...
          "OPT_1.3"          : ("OPT_1.3",         "OPT_1.3",             "OPT_1.3",      [4, 8, 16, 32, 64, 96, 128, 256, 384, 512, 640, 768, 1024], seq_len),
          "OPT_2.7"          : ("OPT_2.7",         "OPT_2.7",             "OPT_2.7",      [4, 8, 16, 32, 64, 96, 128, 256, 384, 512, 640, 768, 1024], seq_len),
          "OPT_6.7"          : ("OPT_6.7",         "OPT_6.7",             "OPT_6.7",      [4, 8, 16, 32, 64, 96, 128, 256, 384, 512, 640, 768, 1024], seq_len),
          "OPT_13"           : ("OPT_13",          "OPT_13",              "OPT_13",       [4, 8, 16, 32, 64, 96, 128, 256, 384, 512, 640, 768, 1024], seq_len),
...
pcie_bws = ["32", "64", "128"]
ssd_bws = ["6.4", "12.8", "19.2", "25.6", "128"]
...
DEFAULT_HOSTMEM_SIZE = "0"
DEFAULT_SSD_BW = "3.2"
DEFAULT_PCIE_BW = "15.754"
DEFAULT_KERNEL_TIME_VARIATION = "0"
DEFAULT_KERNEL_SPEEDUP_VARIATION = "1"

def get_run_config_specification(model: str, netname: str, config_foldername: str, output_foldername: str, policy: str, batch_size: str, seq_len: str, hostmem_size: str=DEFAULT_HOSTMEM_SIZE, pcie_bw : str=DEFAULT_PCIE_BW, ssd_bw : str=DEFAULT_SSD_BW, kernel_time_variation: str=DEFAULT_KERNEL_TIME_VARIATION, kernel_speedup_variation: str=DEFAULT_KERNEL_SPEEDUP_VARIATION):
  characteristics_str = ""
  ...
  # "trans_borden            355" if model == "OPT_1.3" else 
  # "trans_borden            467" if model == "OPT_2.7" else 
  # "trans_borden            467" if model == "OPT_6.7" else 
  # "trans_borden            579" if model == "OPT_13"  else 

  return [f"""
          output_folder           {config_output_folder}
          is_simulation             1
          {"is_transformer            1" if model in ["OPT_1.3", "OPT_2.7", "OPT_6.7", "OPT_13", "BERT", "VIT", "NeRF"] else ""}
          { "trans_borden             210" if model == "BERT" else 
            "trans_borden             184" if model == "VIT" else 
            "trans_borden             7" if model == "NeRF" else 
            ""} 
...
def get_profile_config_specification(model: str, netname: str, config_foldername: str, output_foldername: str, batch_size: str, pf_mode: Enum, is_individual: bool=False, is_cudnn: bool=True):
  ...
  file_pf_string = "InputPF" if pf_mode == PF_MODE.INPUT_PF else "OutputPF" if pf_mode == PF_MODE.OUTPUT_PF else "PF" if pf_mode == PF_MODE.PF else ""
  ...
  #config_output_folder = f"""../results/{output_foldername}/{"cudnn" if is_cudnn else "manual"}{batch_size}{file_pf_string}"""
  #config_output_folder = f"../results/{output_foldername}/{"cudnn" if is_cudnn else "manual"}-{batch_size}{characteristics_str}"
...
def main_performance():
  for model, configs in models.items():
    ...
    for policy in policies:
      for batch_size in batch_sizes:
        content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, policy, batch_size, seq_len)
        with open(filename, "w") as f:
          f.write(content)
    ...
    for policy in cpu_policies:
      for batch_size in batch_sizes:
        ...
        for hostmem_size in temp_hostmem_sizes:
          content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, policy, batch_size, seq_len, hostmem_size=hostmem_size)
          with open(filename, "w") as f:
            f.write(content)
    ...
    for policy in cpu_policies:
      for batch_size in batch_sizes:
          for pcie_bw in pcie_bws:
            content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, policy, batch_size, seq_len, pcie_bw=pcie_bw)
            with open(filename, "w") as f:
              f.write(content)
    ...
    for policy in ssd_policies:
      for batch_size in batch_sizes:
          for ssd_bw in ssd_bws:
            ...
            content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, policy, batch_size, seq_len, pcie_bw=pcie_bw, ssd_bw=ssd_bw)
            with open(filename, "w") as f:
              f.write(content)
  ...
  for model, configs in models.items():
    ...
    for batch_size in batch_sizes:
      for kernel_time_variation in kernel_time_variations:
        content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, "prefetch_lru", batch_size, seq_len, kernel_time_variation=kernel_time_variation)
        with open(filename, "w") as f:
          f.write(content)
    ...
    for policy in kernel_speedup_policies:
      for batch_size in batch_sizes:
        for kernel_speedup_variation in kernel_speedup_variations:
          content, filename = get_run_config_specification(model, netname, config_foldername, output_foldername, policy, batch_size, seq_len, hostmem_size="128", pcie_bw="32", ssd_bw="6.4", kernel_speedup_variation=kernel_speedup_variation)
          with open(filename, "w") as f:
            f.write(content)
