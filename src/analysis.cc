...
std::vector<long> GPU_free_memory_estimation;
...
std::vector<double> normed_kernel_stall;
...
void logMessage(const std::string& message, const std::string& logFileName = "logfile.txt") {
    ...
    // The logFile is automatically closed when the ofstream object is destroyed
}
...
void give_eviction_guide(){
    ...
    logMessage("Eviction guide start");
    ...
    for (int i = 0; i < tensor_list.size(); i++)
    {
        ...
        double ssd_safe_time = 2*(SSD_latency_us + system_latency_us + curr_tensor->size_in_byte / (double)(SSD_bandwidth*1024*1024*1024/1000000));
    ...
    logMessage("Eviction guide finishes");
}
...
//     if(offload_index>ideal_finish_index){
//         logMessage("#2361, offload_index > ideal_index");
//         std::abort();
//     }
...
int ssd2gpu_BWgiveIndx_OverSub(long tensor_size, int start_index){
    long rest_size = tensor_size;
...
int ssd2gpu_BWgiveIndx_half_OverSub(long tensor_size, int offload_index){
    long rest_size = tensor_size / 2;
    int curr_index = offload_index;
    while (rest_size > 0 && curr_index < kernel_list.size())
    {
...
void scheduling_prefetch(){
    logMessage("Prefetch scheduling starts");
    ...
    logMessage("Preallocation starts");
    for (int i = 1; i < tensor_list.size(); i++)
    {
        ...
        if (!curr_tensor->is_global_weight)
        {
            ...
            if (deallo_finish_time_precise < kernel_time_table[kernel_num])
            {
                ...
                if(finish_index < 0){
                    logMessage("#3043, finish_index < 0");
                }
    ...
    logMessage("Preallocation and deallocation finish");
    ...
    logMessage("Pressure region check finishes");
    ...
    for (int i = 1; i < tensor_list.size(); i++)
    {
        ...
        if (!curr_tensor->is_global_weight)
        {
            ...
            if (pre_alloc_start_time_precise < 0)
            {
                ...
                {
                    DataMovementHint pre_allo(PageLocation::NOT_KNOWN, PageLocation::IN_GPU, 0, curr_tensor);
                    pre_allo.human_readable_hint = "Prealloc";
                    movement_hints.push_back(pre_allo);
                }
                issue_index = 0;
            ...
            if (migration_policy_str!="G10GDSSSD" && migration_policy_str!="G10GDSFULL"){
                DataMovementHint pre_dallo(PageLocation::NOT_KNOWN, PageLocation::NOT_PRESENT, death_index, curr_tensor);
                pre_dallo.human_readable_hint = "Predealloc";
                movement_hints.push_back(pre_dallo);
            }
    ...
    logMessage("Predeallocation finishes");
    ...
    // implementation 4 Reassign I/O capacity being aware of oversubscription
    long GPU_total_mem = (long)(GPU_memory_size_GB * 1024 * 1024 * 1024);
    // logMessage("Memory oversubscription aware scheduling initiates");


    
    logMessage("pre-eviction and prefetch scheduling starts");
    ...
    for (int i = 0; i < tot_iter_num; i++)
    {
        ...
        curr_interval->print();
        //logMessage("Current interval list size:" + std::to_string(interval_list.size()));
        logMessage("Current tensor id:" + std::to_string(curr_interval->the_tensor->tensor_id) + " Size: " + std::to_string(curr_interval->the_tensor->size_in_byte) + " Mem: " +std::to_string(curr_interval->GPU_mem_line));
        logMessage("is_offloaded:" + std::to_string(curr_interval->is_offloaded) + " Really?: " + std::to_string(curr_interval->is_really_offloaded));
        
        if (curr_interval->is_offloaded || curr_interval->is_really_offloaded)
        {
            break;
        }
        //For debugging
        // if ((curr_interval->the_tensor->tensor_id) == 1349)
        // {
        //     break;
        // }
        // if ((curr_interval->the_tensor->tensor_id) == 416){

        //     logMessage("eviction, prefetching index");
        //     int kernel_start_index = curr_interval->kernelLevel_interval[0];
        //     int kernel_finish_index = curr_interval->kernelLevel_interval[1];
        //     int pcie_eviction_clear_index = gpu2ssd_BWgiveIndx(curr_interval->the_tensor->size_in_byte, curr_interval->kernelLevel_interval[0]);
        //     int pcie_prefetch_index = ssd2gpu_BWgiveIndx(curr_interval->the_tensor->size_in_byte*1.0, curr_interval->kernelLevel_interval[1]);

        //     int ideal_eviction_clear_index = -1;
        ...
        //     double ssd_movement_estimated_time = ssd_safe_time / 2;
        //     double ideal_eviction_clear_time = kernel_time_table[curr_interval->kernelLevel_interval[0]] + ssd_movement_estimated_time;
        //     for (int j = curr_interval->kernelLevel_interval[0]; j < curr_interval->kernelLevel_interval[1]; j++)
        //         {
        //             if (kernel_time_table[j] <= ideal_eviction_clear_time  && kernel_time_table[j+1] > ideal_eviction_clear_time)
        //             {
        //                 ideal_eviction_clear_index = j;
        //                 break;
        //             }
        //         }

        //     int bwcheck = gpu2ssd_BWcheck(curr_interval->the_tensor->size_in_byte, curr_interval->kernelLevel_interval[0], ideal_eviction_clear_index);
            
        //     logMessage("kernel start index: " + std::to_string(kernel_start_index));
        //     logMessage("kernel finish index: " + std::to_string(kernel_finish_index));
        //     logMessage("BW check clear index: " + std::to_string(bwcheck));
        //     logMessage("ideal eviction clear index: " + std::to_string(ideal_eviction_clear_index));
        //     logMessage("eviction clear index: " + std::to_string(pcie_eviction_clear_index));
        //     logMessage("prefetch start index: " + std::to_string(pcie_prefetch_index));
        //     logMessage("-----------------------------------");
            
        //     // std::cout<<"eviction clear index:"<<pcie_eviction_clear_index<<std::endl;
        //     // std::cout<<"prefetch start index:"<<pcie_prefetch_index<<std::endl;
        //     std::cout<<"-----------------------------------"<<std::endl;
        ...
        if (curr_interval->time_estimated > 0)
        ...
        {   
            ...
            {   
                ...
                if(curr_interval->the_tensor->tensor_id == 1401){
                    logMessage("1401 is not looped");
                    std::stringstream ss;
                    std::streambuf* old_cout_buf = std::cout.rdbuf(ss.rdbuf());
                    curr_interval->print();
                    std::cout.rdbuf(old_cout_buf);  // Restore original buffer
                    ...
                    logMessage("SSD movement time: " + std::to_string(ssd_movement_estimated_time));
                    logMessage("eviction finish time: " + std::to_string(kernel_time_table[curr_interval->kernelLevel_interval[0]] + ssd_movement_estimated_time));
                ...
                // if (eviction_clear_index < 0){
                //     logMessage("#3692 eviction clear index < 0");
                //     std::abort();
                // }
               

                std::cout<<"eviction clear index "<<": "<<eviction_clear_index<<std::endl;
                logMessage("eviction clear index " + std::to_string(eviction_clear_index));
                //NEW: Use PCIe estimation to get the finishing index
                
                //Original code
                int pcie_eviction_clear_index = -2;
                ...
                double prefetch_safe_time_precise = kernel_time_table[curr_interval->kernelLevel_interval[1]] - ssd_prefetch_estimated_time;
                int prefetch_safe_index = -1;
                int prefetch_start_index = -1;
                for (int j = curr_interval->kernelLevel_interval[0]; j < curr_interval->kernelLevel_interval[1]; j++)
                {
                    if (kernel_time_table[j] <= prefetch_safe_time_precise && kernel_time_table[j+1] > prefetch_safe_time_precise)
                    {
                        prefetch_safe_index = j;
                        break;
                    }
                }
                logMessage("prefetch safe index " + std::to_string(prefetch_safe_index));
                //Assert(prefetch_safe_index>=0);

                if(prefetch_safe_index < 0 || eviction_clear_index < 0){
                    curr_interval->is_offloaded = true;
                    break;
                }
                ...
                //implementation 6
                // double rest_size = curr_interval->the_tensor->size_in_byte;
                // for (int j = curr_interval->kernelLevel_interval[1]-1; j > prefetch_safe_index; j--)
                // {   
                //     if (GPU_resident_memory_estimation[j] - GPU_total_mem > 0) //if memory oversubscription occurs
                //     {
                //         std::vector<Tensor*> r;
                //         kernel_list[j+1].getRequiredTensors(r);
                //         long over_tensor_size = 0;

                //         for (int k = 0; k<r.size(); k++)
                //         {
                //             if(!r[k]->is_global_weight){
                //                 over_tensor_size = r[k]->size_in_byte;
                //             }
                //         }
                //         rest_size -= over_tensor_size;
                //     }
                ...
                {   
                    ...
                    //implementation 6: scheduling migration only if tensors reduce oversubscription overhead
                    long current_memory = 0;
                    double overSub_delay = 0;
                    double prefetch_delay = 0;
                    double delay_diff = 0;
                    double delay_sum = 0;
                    double lowest_diff_sum = std::numeric_limits<double>::max();
                    for (int m = eviction_clear_index+1; m < curr_interval->kernelLevel_interval[1]; m++)
                    {   
                        overSub_delay = 0;
                        if (kernel_time_table[m] > kernel_time_table[prefetch_safe_index])
                        {
                            prefetch_delay = kernel_time_table[m] - kernel_time_table[prefetch_safe_index];
                        }
                        
                        std::vector<long> temp_memory_estimation(GPU_resident_memory_estimation);
                        std::vector<DataMovementHint> temp_hints(movement_hints);
                        sort(temp_hints.begin(), temp_hints.end());

                        for (int n = eviction_clear_index; n < m; n++)
                        {
                            temp_memory_estimation[n] -= curr_interval->the_tensor->size_in_byte;

                            if (temp_memory_estimation[n] > GPU_total_mem)
                            {
                                //overSub_delay += ssd_movement_estimated_time;
                                for(int p = 0; p < temp_hints.size(); p++){
                                    if(temp_hints[p].issued_time == n && temp_hints[p].human_readable_hint == "Prealloc")
                                    {
                                        overSub_delay += SSD_latency_us + system_latency_us + temp_hints[p].tensor->size_in_byte / (double)(SSD_bandwidth*1024*1024*1024/1000000);
                                    }

                                    if(temp_hints[p].issued_time > n){
                                        break;
                                    }
                                }
                            }
                        ...
                        delay_sum = overSub_delay + prefetch_delay;
                        if(delay_sum < lowest_diff_sum)
                        {
                            lowest_diff_sum = delay_sum;
                            prefetch_start_index = m;
                            delay_diff = overSub_delay - prefetch_delay;
                        }
                        
                    }
                    if(prefetch_start_index = -1)
                    {
                        prefetch_start_index = prefetch_safe_index;
                    }
                    // 1. calculate prefetch delay
                    //double prefetch_delay = 0;

                    
                    // 2. update the GPU memory profile

                    
                    if(lowest_diff_sum > 0 && delay_diff > 0)
                    {
                        //First schedule the pre-eviction
                        ...
                        movement_hints.push_back(pre_evict);
                        curr_interval->the_tensor->is_choosed_to_evict = true;
                        curr_interval->is_really_offloaded = 1;
                        curr_interval->is_offloaded = 1;

                        // DataMovementHint pre_fetch(PageLocation::NOT_KNOWN, PageLocation::IN_GPU, pcie_prefetch_index, curr_interval->the_tensor);
                        // movement_hints.push_back(pre_fetch);
                        curr_interval->original_prefetch_index = prefetch_start_index;
                        curr_interval->evict_finish_index = eviction_clear_index;
                        offloeded_local_intervals.push_back(curr_interval);
            ...
            {    
                ...
                double eviction_finish_time_loop = kernel_time_table[curr_interval->kernelLevel_interval[0]] + ssd_movement_estimated_time;

                int eviction_clear_index_loop = -1;
                ...
                {   
                    if (j==curr_interval->kernelLevel_interval[0]){
                    }
                    if (kernel_time_table_extended[j] <= eviction_finish_time_loop && kernel_time_table_extended[j+1] > eviction_finish_time_loop)
                    {
                        eviction_clear_index_loop = j;
                        break;
                    }
                }
                //for debugging
                ...
                // if (eviction_clear_index< 0){
                //     logMessage("#3697 eviction_clear_index < 0");
                //     std::abort();
                // }
                
                //Assert(eviction_clear_index >= 0 && "Eviction clear index is higher than 0 - 3496");
                std::cout<<"eviction clear index "<<": "<<eviction_clear_index_loop<<std::endl;

                //Second schedule the prefetch
                double prefetch_start_time_precise_loop = kernel_time_table_extended[curr_interval->kernelLevel_interval[1] + kernel_num] - ssd_prefetch_estimated_time;
                int prefetch_start_index_loop = -1;
                for (int j = curr_interval->kernelLevel_interval[0]; j < curr_interval->kernelLevel_interval[1] + kernel_num; j++)
                {
                    if (kernel_time_table_extended[j] <= prefetch_start_time_precise_loop && kernel_time_table_extended[j+1] > prefetch_start_time_precise_loop)
                    {
                        prefetch_start_index_loop = j;
                        break;
                    }
                }
                ...
                if (eviction_clear_index_loop < 0 || prefetch_start_index_loop < 0 || prefetch_start_index_loop < eviction_clear_index_loop){
                    curr_interval->is_offloaded = true;
                    break;
                }
                
                //Assert(prefetch_start_index>=0 && "Prefetch start index is higher than 0-3510");
                std::cout<<"prefetch start index "<<": "<<prefetch_start_index_loop<<std::endl;
                if (prefetch_start_index_loop!=curr_interval->kernelLevel_interval[0])
                {
                    //First schedule the pre-eviction
                    DataMovementHint pre_evict(PageLocation::NOT_KNOWN, PageLocation::IN_SSD, curr_interval->kernelLevel_interval[0], curr_interval->the_tensor);
                    pre_evict.human_readable_hint = "Preevict";
                    ...
                    DataMovementHint pre_fetch(PageLocation::NOT_KNOWN, PageLocation::IN_GPU, prefetch_start_index_loop % kernel_num, curr_interval->the_tensor);
                ...
                //minus mem
                for (int j = eviction_clear_index_loop + 1; j < prefetch_start_index_loop; j++)
                {
        ...
        else if (curr_interval->time_estimated > cpu_safe_time)
        {
            ...
            else
            {
                ...
                if (eviction_clear_index< 0){
                    logMessage("#3835 Eviction clear index< 0");
                    std::abort();
                }
    ...
    logMessage("First-time offloading finishes");
    print_GPU_mem_estimation();

    if (eviction_policy_str=="TOLERANT")
    {
        // Cold periods second pass, for smart eviction policy
        std::cout << "tolearnt eviction policy" << std::endl;
        for (int i = 0; i < interval_list.size(); i++)
        {
            ...
            if (curr_interval->time_estimated > cpu_safe_time)
            {
                ...
                if (!curr_interval->is_looped)  //Not looped
                {
                    ...
                    Assert(eviction_clear_index >= 0 && "Eviction clear index is higher than 0-3755");
                    ...
                    Assert(prefetch_start_index>=0 && "Prefetch satart index is higher than 0-3769");
                    ...
                    Assert(un_pin_index>=0 && "unpin index is higher than 0-3783");
                ...
                else
                {
                    ...
                    Assert(eviction_clear_index >= 0 && "Eviction clear index is higher than 0-3821");
                    ...
                    Assert(prefetch_start_index>=0 && "prefetch start is higher than 0-3835");
                    ...
                    Assert(un_pin_index>=0 && "unpin index is higher than 0-3849");
    ...
    double params_alpha = 1;
    long prefetch_threshold = params_alpha * target_mem_line;
    for (int i = 0; i < offloeded_local_intervals.size(); i++)
    {
        ...
        while (iindx > current_interv->evict_finish_index + 1)
        {
            ...
            if (prefetch_threshold > GPU_resident_memory_estimation[iindx - 1])
            {
                iindx--;
            }
            else
    ...
    logMessage("Second-time Modification finishes");
    print_GPU_mem_estimation();
    ...
    logMessage("Sorting interval finishes");
    if (migration_policy_str=="G10GDSSSD" || migration_policy_str=="G10GDSFULL")
    {
...
void print_prefetch_table(){
    for (int i = 0; i < movement_hints.size(); i++)
    {
        ...
        std::cout << "Issued Time: " << curr.issued_time << " ";
        std::cout << "Event: " << curr.human_readable_hint << " ";
...
void print_stall(){
    for (int i = 0; i < normed_kernel_stall.size(); i++)
    { 
        std::cout << "Stall time: " << normed_kernel_stall[i]  << std::endl;
    }
...
void FlashNeuron_simulator::run(){
    ...
    for (int i = 0; i < kernel_list.size(); i++)
    {
        ...
        normed_kernel_stall.push_back((kernel_finish_time-kernel_start_time-kernel_exe_time)/kernel_exe_time);
    }
