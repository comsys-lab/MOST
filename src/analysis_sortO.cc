...
void scheduling_prefetch(){
    ...
    for (int i = 0; i < tot_iter_num; i++)
    {
        ...
        //         // implementation 3, find the best prefetching index with minimal oversubscription - prefetching delay
        //         offload_mid_index = gpu2ssd_BWgiveIndx_half(a->the_tensor->size_in_byte, a->kernelLevel_interval[0]);

        //         double ssd_safe_time = 2*(SSD_latency_us + system_latency_us + a->the_tensor->size_in_byte / (double)(SSD_PCIe_bandwidth_GBps*1024*1024*1024/1000000));
        //         double ssd_movement_estimated_time = ssd_safe_time / 2;

        //         long oversubscription_size = 0;
        ...
        //         int optimal_prefetch_index = -1;
        //         int optimal_prefetch_mid_index = -1;
        //         // Searching prefetch start index, which shows minial delay
        //         for (int j = offload_mid_index; j < a->kernelLevel_interval[1]; j++)
        //         {   
        //             std::vector<long> temp_memory_estimation(GPU_resident_memory_estimation);
        //             //prefetch mid index => safe index

        //             // 1. Find an index that finishes prefetching
        //             int prefetch_finish_index = ssd2gpu_BWgiveIndx_half_OverSub(a->the_tensor->size_in_byte, j);
                    
        //             // 2. update the GPU memory profile
        //             for (int m = offload_mid_index; m < prefetch_finish_index; m++)
        //             {   
        //                 temp_memory_estimation[m] -= a->the_tensor->size_in_byte;
        //             }

        //             // 3. Caclulate memory oversubscription overhead after pre-eviction and prefetching
        //             for (int n = offload_mid_index; n < kernel_time_table.size(); n++)
        //             {
        //                 if (temp_memory_estimation[n] - a->GPU_mem_line > 0)
        //                 {
        //                     oversubscription_size += temp_memory_estimation[n] - a->GPU_mem_line;
        //                 }
        //             }
        //             oversubscription_delay = oversubscription_size / SSD_PCIe_bandwidth_GBps;
        //             double prefetch_safe_time = kernel_time_table[a->kernelLevel_interval[1]] - ssd_movement_estimated_time;
        ...
        //                     optimal_prefetch_index = j; // prefetch start index
        //                 }
        //             }
        //         }

        //         optimal_prefetch_mid_index = ssd2gpu_BWgiveIndx_half_OverSub(a->the_tensor->size_in_byte, optimal_prefetch_index);
                        
        //         for (int j = offload_mid_index; j < optimal_prefetch_mid_index; j++)
        //         {
        ...
        //         double ssd_safe_time = 2*(SSD_latency_us + system_latency_us + b->the_tensor->size_in_byte / (double)(SSD_PCIe_bandwidth_GBps*1024*1024*1024/1000000));
        //         double ssd_movement_estimated_time = ssd_safe_time / 2;
        ...
        //             // 1. Find an index that finishes prefetching
        //             int prefetch_finish_index = ssd2gpu_BWgiveIndx_half_OverSub(b->the_tensor->size_in_byte, j);
                    
        //             // 2. update the GPU memory profile
        //             for (int m = offload_mid_index; m < prefetch_finish_index; m++)
        //             {   
        //                 temp_memory_estimation[m] -= b->the_tensor->size_in_byte;
        //             }

        //             // 3. Caclulate memory oversubscription overhead after pre-eviction and prefetching
        //             for (int n = offload_mid_index; n < kernel_time_table.size(); n++)
        //             {
        //                 if (temp_memory_estimation[n] - b->GPU_mem_line > 0)
        //                 {
        //                     oversubscription_size += temp_memory_estimation[n] - b->GPU_mem_line;
        //                 }
        //             }
        //             oversubscription_delay = oversubscription_size / SSD_PCIe_bandwidth_GBps;
        //             double prefetch_safe_time = kernel_time_table[b->kernelLevel_interval[1]] - ssd_movement_estimated_time;
        //             prefetch_delay = kernel_time_table[prefetch_finish_index] - prefetch_safe_time;
        ...
        //         optimal_prefetch_mid_index = ssd2gpu_BWgiveIndx_half_OverSub(b->the_tensor->size_in_byte, optimal_prefetch_index);
        ...
        {   
            ...
            {   
                ...
                {
                    std::cout<<"----------------------------------------------------"<<std::endl;
                    std::cout<<"Error occurs when scheduling eviction:"<<eviction_clear_index<<std::endl;
                    std::cout<<"----------------------------------------------------"<<std::endl;
                }
