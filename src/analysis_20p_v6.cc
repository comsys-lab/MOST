...
void scheduling_prefetch(){
    ...
    // {
    //     // get oversubscription overhead caused from allocating next kernel inputs
    //     long over_mem = 0;
    //     std::vector<Tensor*> r;
    //     kernel_list[i+1].getRequiredTensors(r);
    //     long tensor_size = 0;
    //     for (int j = 0; j < r.size(); j++)
    //     {
    //         if(!r[j]->is_global_weight){
    //             tensor_size = r[j]->size_in_byte;
    //         }
    //     }

    //     if(GPU_resident_memory_estimation[i] > GPU_total_mem)
    //     {
    //         over_mem = tensor_size;
    //     }
    //     else
    //     {
    //         if(GPU_resident_memory_estimation[i] + tensor_size > GPU_total_mem)
    //         {
    //             over_mem = GPU_resident_memory_estimation[i] + tensor_size - GPU_total_mem;
    //         }
    //         else{
    //             over_mem = 0;
    //         }
    //     }
    //     ssd2gpu_BW_estimation[i].capacity += over_mem * (over_mem/(SSD_bandwidth*1024*1024*1024));
    // }
    ...
    for (int i = 0; i < tot_iter_num; i++)
    {
        ...
        //         area_can_reduce_a = a->the_tensor->size_in_byte * a->time_estimated;
        ...
        //         area_can_reduce_b = b->the_tensor->size_in_byte * b->time_estimated;
        ...
        {   
            ...
            {   
                ...
                {   
                    ...
                    //implementation 6
                    // if (pcie_prefetch_index< 0){
                    // logMessage("#3449 eviction clear index < 0");
                    // std::abort();
                    // }
                    ...
                    // if (pcie_eviction_clear_index< 0){
                    // logMessage("#3540 pcie_eviction_clear_index < 0");
                    // std::abort();
                    // }
                    ...
                    // implementation 3: memory-oversubscription-aware migration scheduling
                    ...
                    //     int oversubscription_size = 0;
                    //     double oversubscription_delay = 0;
                    //     double prefetch_delay = 0;
                    //     double delay_diff = 0;
                    //     double lowest_diff_delay = std::numeric_limits<double>::max();
                    //     int lowest_diff_index = -1;

                    //     // Searching prefetch start index, which shows minial delay
                    //     for (int j = pcie_eviction_clear_index; j < curr_interval->kernelLevel_interval[1]; j++)
                    //     {   
                    //         std::vector<long> temp_memory_estimation(GPU_resident_memory_estimation);
                            

                    //         // 1. Find an index that finishes prefetching
                    //         int prefetch_finish_index = ssd2gpu_BWgiveIndx_OverSub(curr_interval->the_tensor->size_in_byte, j);
                            
                    //         // 2. update the GPU memory profile
                    //         for (int m = pcie_eviction_clear_index; m < j; m++)
                    //         {   
                    //             temp_memory_estimation[m] -= curr_interval->the_tensor->size_in_byte;
                    //         }

                    //         for (int m = prefetch_finish_index; m < kernel_time_table.size(); m++)
                    //         {   
                    //             temp_memory_estimation[m] += curr_interval->the_tensor->size_in_byte;
                    //         }
                            
                    //         // 3. Caclulate memory oversubscription overhead after pre-eviction and prefetching
                    //         for (int n = pcie_eviction_clear_index; n < kernel_time_table.size(); n++)
                    //         {
                    //             if (temp_memory_estimation[n] - GPU_total_mem > 0)
                    //             {
                    //                 oversubscription_size += temp_memory_estimation[n] - GPU_total_mem;
                    //             }
                    //         }
                    //         oversubscription_delay = oversubscription_size / SSD_bandwidth;
                    //         double prefetch_safe_time = kernel_time_table[curr_interval->kernelLevel_interval[1]] - ssd_movement_estimated_time;
                    //         prefetch_delay = kernel_time_table[prefetch_finish_index] - prefetch_safe_time;

                    //         // 4. Compare the delays and schedule eviction and prefetching 
                    //         // if oversubscription overhead exceeds prefetching delay
                    //         if (prefetch_delay > oversubscription_delay)
                    //         {
                    //             continue;
                    //         }
                    //         else{
                    //             delay_diff = oversubscription_delay - prefetch_delay;
                    //             if(delay_diff < lowest_diff_delay)
                    //             {
                    //                 lowest_diff_delay = delay_diff;
                    //                 pcie_prefetch_index = j;
                    //             }
                    //         }
                    //     }
                    ...
                    //         std::cout<<"Ours tensor id"<<": "<<curr_interval->the_tensor->tensor_id<<std::endl;
                    //         std::cout<<"Scheduled eviction kernel"<<": "<<pcie_eviction_clear_index<<std::endl;
                    //         std::cout<<"Scheduled prefetching kernel"<<": "<<pcie_prefetch_index<<std::endl;
                    //         std::cout<<"Saved time"<<": "<<lowest_diff_delay<<std::endl;
                    //     }
                    ...
                    // 1. update the GPU memory profile

                    long area_reduce = 0;
                    for (int m = eviction_clear_index; m < prefetch_safe_index; m++)
                    {   
                        temp_memory_estimation[m] -= curr_interval->the_tensor->size_in_byte;
                    }
                    ...
                    for (int m = eviction_clear_index; m < prefetch_safe_index; m++)
                    {   
                        if (temp_memory_estimation[m] > GPU_total_mem)
                        {
                            area_reduce += temp_memory_estimation[m] - GPU_total_mem;
                        }
                    }
                    ...
                    if(area_reduce > 0){
                        ...
                        // DataMovementHint pre_fetch(PageLocation::NOT_KNOWN, PageLocation::IN_GPU, pcie_prefetch_index, curr_interval->the_tensor);
                        // movement_hints.push_back(pre_fetch);
                        curr_interval->original_prefetch_index = prefetch_safe_index;
                    ...
                    for (int j = eviction_clear_index + 1; j < prefetch_safe_index; j++){
                ...
                // if (pcie_eviction_clear_index< 0){
                //     logMessage("#3663 pcie_eviction_clear_index < 0");
                //     std::abort();
                // }
                ...
                //     long before_scheduling_mem = GPU_resident_memory_estimation[j];
                //     GPU_resident_memory_estimation[j] -= curr_interval->the_tensor->size_in_byte;
                //     long after_scheduling_mem = GPU_resident_memory_estimation[j];

                //     if(before_scheduling_mem > GPU_total_mem && after_scheduling_mem < GPU_total_mem){
                //         std::vector<Tensor*> r;
                ...
                //                 tensor_size = r[k]->size_in_byte;
                //             }
                //         }

                //         ssd2gpu_BW_estimation[j].capacity -= tensor_size * (tensor_size/(SSD_bandwidth*1024*1024*1024));
                //     }
            ...
            {    
                ...
                if (prefetch_start_index< 0){
                    logMessage("#3713 prefetch start index < 0");
                    std::abort();
                }
